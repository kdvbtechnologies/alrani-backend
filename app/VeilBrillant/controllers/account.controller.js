  ...
  try {
    ...
    upload(req, res, (err) => {
      ...
      cloudinary.uploader.upload_large(
        ...
        {
          resource_type: "video",
          public_id: `Veil Brillant 1 - Videos/${fName}`,
          chunk_size: 6000000,
          eager: [
            {
              width: 300,
...
/*module.exports.updateUserInfos = async (req, res) => {
  ...
  const { nomAuteur, telephone, password, photoProfil } = req.body;
...
// url Photo
// url Photo
/*module.exports.urlPhoto = async (req, res) => {
  try {
    // ici on reduit le poids de l'image sans perdre la qualité
    const data = await sharp(req.file.buffer).webp({ quality: 20 }).toBuffer();
    const stream = cloudinary.uploader.upload_stream(
      { folder: "Veil Brillant - Photos" },
      (error, result) => {
        if (error) return console.error(error);
        // ici on renvoie l'URL de l'image après l'upload sur cloudinary
        return res.json({ urlPhoto: result.secure_url });
